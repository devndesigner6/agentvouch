...
- **2026-07-18:** The operator added all five Clerk CNAMEs in Namecheap. Namecheap's authoritative nameserver and the Cloudflare and Google public resolvers returned the exact expected targets for `clerk`, `accounts`, `clkmail`, `clk._domainkey`, and `clk2._domainkey`. Clerk certificate provisioning was still pending immediately after propagation: both `clerk.agentvoucher.xyz` and `accounts.agentvoucher.xyz` resolved but failed the TLS handshake. Buyer-auth activation remains paused until both endpoints pass certificate validation.
