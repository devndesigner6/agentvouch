...
abstract contract A1DeploymentConfig is LinkedLibraryVerifier {
    ...
    function _assertBaseSepoliaUsdc(address usdc) internal view {
        if (block.chainid != BASE_SEPOLIA_CHAIN_ID && block.chainid != ROBINHOOD_TESTNET_CHAIN_ID && !_isLocalRehearsal()) {
            revert WrongChain(ROBINHOOD_TESTNET_CHAIN_ID, block.chainid);
        }
        if (usdc.code.length == 0 && !_isLocalRehearsal()) revert InvalidA1DeploymentInput("USDC_ADDRESS_CODE");
        if (!_isLocalRehearsal() && IERC20Metadata(usdc).decimals() != 6) revert InvalidA1DeploymentInput("USDC_DECIMALS");
    }
        ...
        returns (AgentVouchTypes.Config memory c)
    {
        ...
        c.usdc = usdc;
        c.chainContext = block.chainid == ROBINHOOD_TESTNET_CHAIN_ID ? "eip155:46630" : "eip155:84532";
