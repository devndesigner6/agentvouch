...
export async function ensureBaseSepoliaInjectedChain(
  ...
  const targetChainIdHex = numberToHex(config.chainId);
  const chainId = await provider.request({ method: "eth_chainId" });
  if (chainId === targetChainIdHex) return;
  if (chainId === "0x2105") {
    throw new Error(
      `Base mainnet is not enabled for AgentVouch writes. Switch MetaMask to ${config.chainLabel}.`
    );
  }
  if (options.requestSwitch === false) {
    throw new Error(`MetaMask is not connected to ${config.chainLabel}.`);
  }
  ...
  if (activeChainId !== targetChainIdHex) {
    throw new Error(`MetaMask did not switch to ${config.chainLabel}.`);
  }
}
...
export async function probeBaseInjectedExecutionMode(
  ...
  try {
    const capabilities = await provider.request({
      ...
      params: [address, [targetChainIdHex]],
    });
    if (
      capabilities &&
      typeof capabilities === "object" &&
      targetChainIdHex in capabilities
    ) {
