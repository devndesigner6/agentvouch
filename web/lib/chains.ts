...
export const ROBINHOOD_TESTNET_CHAIN_CONTEXT = "eip155:46630";
...
export const CHAIN_ALIASES: Record<string, string> = {
  ...
  [ROBINHOOD_TESTNET_CHAIN_CONTEXT]: "Robinhood Testnet",
...
const LEGACY_CHAIN_CONTEXTS: Record<string, string> = {
  ...
  "base:sepolia": BASE_SEPOLIA_CHAIN_CONTEXT,
  "robinhood-testnet": ROBINHOOD_TESTNET_CHAIN_CONTEXT,
  "robinhood:testnet": ROBINHOOD_TESTNET_CHAIN_CONTEXT,
  robinhood: ROBINHOOD_TESTNET_CHAIN_CONTEXT,
  ...
  if (!normalized) {
    warnDefaultChainContextOnce(
      `Ignoring invalid NEXT_PUBLIC_AGENTVOUCH_DEFAULT_CHAIN_CONTEXT "${configured}"; ` +
        `defaulting to ${ROBINHOOD_TESTNET_CHAIN_CONTEXT}.`
    );
    return ROBINHOOD_TESTNET_CHAIN_CONTEXT;
  }

  if (normalized === ROBINHOOD_TESTNET_CHAIN_CONTEXT) return normalized;
...
export function isRobinhoodTestnetDefaultEnabled(): boolean {
  return getDefaultChainContext() === ROBINHOOD_TESTNET_CHAIN_CONTEXT;
}
